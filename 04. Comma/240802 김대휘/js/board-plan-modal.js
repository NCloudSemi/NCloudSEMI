...
function toggleEditMode(isEditable) {
    ...
    if (isEditable) {
        ...
        document.querySelectorAll('.map-box-preview-item').forEach(item => {
            ...
            item.querySelectorAll('*').forEach(el => el.style.pointerEvents = '');
        });
        ...
        document.getElementById('chapter-add').style.pointerEvents = 'auto';
    } else {
        disableDragAndDrop(); // 드래그 앤 드롭 비활성화
        document.querySelectorAll('.map-box-preview-item').forEach(item => {
            item.setAttribute('draggable', false);
            item.querySelectorAll('*').forEach(el => el.style.pointerEvents = 'none');
        });
        ...
        document.getElementById('chapter-add').style.pointerEvents = 'none';
    }
}
...
function handleTimelineBoxDrop(event) {
    ...
    if (!timelineBox.contains(event.target)) {
        ...
        if (confirm('정말 해당 계획을 삭제하시겠습니까? 작성하신 내용은 저장되지 않습니다.')) {
            ...
            draggedElement.remove();
            timelineItemCount--;
            updateTimelineNumbers(startLocationInput); // 출발지 입력값을 인자로 전달
        } else {
            // 취소한 경우 원래 위치로 되돌림
        ...
        if (targetColumn) {
            ...
            draggedElement.style.display = '';
            updateTimelineNumbers(); // 출발지 입력값을 복원할 필요 없음
        } else {
